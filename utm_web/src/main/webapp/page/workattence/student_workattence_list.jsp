...
<custom:paginationForm action="${contextPath}/workattence/queryStudentWorkattenceAction.action">
	<input type="hidden" name="workattence.studentByStudentid.code" value="${workattence.studentByStudentid.code}"/>
    <input type="hidden" name="workattence.studentByStudentid.name" value="${workattence.studentByStudentid.name}"/>
...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="${contextPath}/workattence/queryStudentWorkattenceAction.action" method="post">
	<div class="searchBar">
        <table class="searchContent">
            <tbody><tr>
                <td>
                    卡号：
                    <input type="text" name="workattence.studentByStudentid.code" value="${workattence.studentByStudentid.code}" maxlength="20" />
                </td>
                <td>
                    姓名：
                    <input type="text" name="workattence.studentByStudentid.name" value="${workattence.studentByStudentid.name}" maxlength="20" />
                </td>
                <td>是否可用:
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" width="440" href="${contextPath}/workattence/showAddStudentWorkattenceAction.do" target="dialog"  mask="true"><span>添加</span></a></li>
			<li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids" href="${contextPath}/workattence/deleteStudentWorkattenceByArrayAction.do" class="delete" warn="请选择用户！"><span>删除</span></a></li>
			<li><a class="edit" width="440" href="${contextPath}/workattence/showUpdateStudentWorkattenceAction.do?workattence.id={sid_workattence}" target="dialog"  mask="true" warn="请选择一个用户！"><span>修改</span></a></li>
			<li class="line">line</li>
            <li><a class="icon"  href="${contextPath }/workattence/exportStudentWorkattenceAction.do" target="dwzExport" title="确定要导出这些记录吗?"><span>导出EXCEL</span></a></li>
		</ul>
	</div>
		...
		<tbody>
			...
			<tr target="sid_workattence" rel="${workattence.id}">
			  <td><input name="ids" value="${workattence.id}" type="checkbox"></td>
							...
							<td>${workattence.studentByStudentid.state}</td>
					<td>
				    <a title="确定要删除这条记录吗？" target="ajaxTodo" href="${contextPath}/workattence/deleteStudentWorkattenceByIdAction.do?workattence.id=${workattence.id}" class="btnDel">删除</a>
					<a title="编辑"  target="dialog" width="440" mask="true" href="${contextPath}/workattence/showUpdateStudentWorkattenceAction.do?workattence.id=${workattence.id}"class="btnEdit">编辑</a>
				</td>
			</tr>
			</c:forEach>
		</tbody>
